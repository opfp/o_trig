...
struct func_set o_trig_obj = {0}; 

const struct function_des FUNC_DES[] = { 
...
int o_trig_init(char * table_fp, int tbs_to_load ) { 
	...
	*/
	...
	o_trig_obj.contents = 5;
	make_tables(&o_trig_obj); 
} 

/* 
    * @scope public 
    * @breif perfroms a table lookup to find an aproximation for infunc( inval ). Then, linearly 
    *       scales the result according to the distance between the inval and closest matching x 
    *       in the lookup table, and the slope of the line between infunc (aprox inval) and 
    *       infunc ( aprox inval +/- 1) 
...
float o_trig_lookup(enum func infunc, float inval) { 
    ...
    struct function_des * fdes = FUNC_DES[infunc]; 
    // check inval in function range 
    if ( inval < fdes->real_range_min || inval > fdes->real_range_max ) { 
        fprintf("X %f out of range for function %s with range (%f, %f)\n", 
    ...
    if ( inval < fdes->table_range_min || inval > fes->table_range_max ) { 
    ...
    int match_i = ltable_bsearch(o_trig_obj[fdes->table], trans_inval, infunc%2, fdes->ascending ); 

}

/* 
    * @scope public 
    * @breif Performs a table lookup like o_trig_init, but skips the linear scaling step to 
    *       more quicly return a result 
    * @param infunc The function 
    * @param inval The input value 
*/

float o_trig_quick_lookup(enum func infunc, float inval) { 
	// get index of closent match to inval 
	int tb_i = infunc >> 1; // evens are function 
	int offset = infunc & 1; // odds are inverse of that function (sine, arcsine . . . ) 
	char ascending = infunc != ARC_COSINE && infunc != COSINE; 
			
	// return (func_set) o_trig_obj.tables[o_trig_ltable_bsearch(o_trig_obj.tables[tb_i], inval, offset, ascending)+1]; 
} 
...
int gen_lookup_table( struct table_set * dest ) { 
	...
	float retry_inc_constant = -1 / 0x100 ; 
	...
	for ( int i = 0; i < point_denisty; i++ ) { 
		// find point on circle at this height 
		cx = px - x_inc; 
		cy = py + y_inc; 
		
		c_dif = DIST(0,0,cx,cy) - 1.0;
		while ( (int) ( c_dif * accuracy ) ) { // while point not close enough to circle  
			cx += c_dif * retry_inc_constant;
			c_dif = ORIGIN_DIST(cx,cy) - 1.0;
		}  
		...
		if ( tables_to_make & ( 1 << TB_SINE_COS ) ) { 
			o_trig_obj[TB_SINE_COS][i*2] = cy / hyp_len; 
			o_trig_obj[TB_SINE_COS][(i*2)+1] = c_circ_len; 
		} 
...
//		} 
		if ( tables_to_make & ( 1 << TB_TAN ) ) { 
			o_trig_obj[tangent][i*2] = cx / cy; 
			o_trig_obj[tangent][(i*2)+1] = c_circ_len; 
		} 		 
...
int ltable_bsearch( float * table, float search_val, int offset, char ascending) { 
	int search_inc = o_trig_obj.point_desnity / 2; 
	unsigned si = o_trig_obj.point_density; // / 2 ; 
	// check edge cases
	int i_high = ascending ? (point_density*2)-2+offset : offset;   
	int i_low = ascending ? offset : (point_density*2)-2+offset; 	
...
struct transform_result * trans_sine(float x_in, float * p_x_trans, float * p_mirror_ab ) { 
    float mirror = NAN; 
...
struct transform_result * trans_cosine(float x_in, float * p_x_trans, float * p_mirror_ab) { 
...
struct transform_result * trans_arc_cosine(float y_in, float * p_x_trans, float * p_mirror_ab) { 
    *p_x_trans = y_in; 
    *p_mirror_ab = M_PI_4; 
} 
...
struct transform_result * trans_tan(float x_in, float * p_x_trans, float * p_mirror_ab) { 
    float x_trans = fmod( ( x_in < 0 ) ? -1.0 * x_in , M_PI ); 
